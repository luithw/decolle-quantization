...
fixed_quantizers[8] = quantizer(forward_number=FixedPoint(wl=8, fl=7, clamp=True),
                             ...
                             backward_rounding="stochastic",
                             clamping_grad_zero=True)
fixed_quantizers[4] = quantizer(forward_number=FixedPoint(wl=4, fl=3, clamp=True),
                             forward_rounding="stochastic",
                             backward_rounding="stochastic",
                             clamping_grad_zero=True)
relu = torch.relu 
...
def parse_args(default_params_file = 'parameters/params.yml'): 
    ...
    parser.add_argument('--quantization', dest='quantization', action='store_true', help='Flag to enable quantization')
...
def prepare_experiment(name, args): 
    ...
    if 'online_update' not in params:
    ...
    if 'stride' not in params:
        print('Stride is not explicitly defined, assuming 1')
        params['stride'] = [1]

    if 'lif_layer_type' not in params:
        print('lif_layer_type is not explicitly defined, assuming LIFLayer')
        params['lif_layer_type'] = 'LIFLayer'

    if 'need_bias' not in params:
        print('need_bias is not explicitly defined, assuming True')
        params['need_bias'] = True

    if 'precision' not in params:
        print('precision is not explicitly defined, assuming 16')
        params['precision'] = [16] * (params['num_conv_layers'] + params['num_mlp_layers'])
...
def test(gen_test, decolle_loss, net, burnin, print_error = True, debug = False): 
    ...
    with torch.no_grad(): 
        ...
        if print_error:
            print(' '.join(['Test Accuracy L{0} {1:1.3}'.format(j, v) for j, v in enumerate(test_acc)]))
...
class MultiOpt(object): 
    ...
    def load_state_dict(self, state_dicts):
        ...
        if len(state_dicts) == len(self.optimizers):
            ...
            print("Optimizer state_dict is not compatible with MultiOpt, skipping.")
...
def train_timewrapped(gen_train, loss_fn, net, opt, epoch, online_update=False, batches_per_epoch=-1):
    '''
    Trains a DECOLLE network
    ...
    for data_batch, target_batch in tqdm.tqdm(iter_gen_train, desc='Epoch {}'.format(epoch)):
        ...
        for i in range(len(net)):
            act_rate[i] += tonp(u[i].mean().data)/t_sample
...
def extract_net_states(net):
    """Extra all neuron states to a list."""

    states = []
    for lif_layer in net.LIF_layers:
        clean_states = {}
        for name, internal_state in lif_layer.state._asdict().items():
            clean_states[name] = torch.zeros_like(internal_state)
        states.append(lif_layer.NeuronState(**clean_states))
    return states
